...
class Pointnet2MSG(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=3, use_xyz=True):
        ...
        self.SA_modules.append(
            PointnetSAModuleMSG(
                npoint=512, #点的数量 512
                radii=[0.1, 0.2, 0.4], #采样的不同尺度的半径
                nsamples=[16, 32, 128], #每个采样半径对应的局部区域内的点数
                mlps=[
        ...
        input_channels = 64 + 128 + 128 #将不同尺度的特征级联起来
        self.SA_modules.append(
            PointnetSAModuleMSG(
                npoint=128,
                radii=[0.2, 0.4, 0.8],
    ...
    def forward(self, pointcloud):
        ...
        """
        xyz, features = self._break_up_pc(pointcloud) #第一个samodule中features=None
        ...
        for module in self.SA_modules:
            ...
            print('main feature', features.shape)
...
if __name__ == "__main__":
    ...
    model = Pointnet2MSG(40, input_channels=0, use_xyz=True).cuda()
    # preds = model(inputs, [512,128,64])
    #preds = model(inputs,[512, 128])
    print(model)
    preds = model(inputs)
